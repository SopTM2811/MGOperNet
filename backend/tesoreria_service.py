...
class TesoreriaService:
    ...
    def _generar_cuerpo_correo(self, lote_info: Dict, solicitudes: List[Dict]) -> str:
        ...
        cuerpo += f"<li><strong>Total capital a dispersar (ligas):</strong> ${lote_info['total_capital']:,.2f}</li>"
        cuerpo += f"<li><strong>Total comisión DNS (0.375% capital):</strong> ${lote_info['total_comision_dns']:,.2f}</li>"
        cuerpo += f"<li><strong>TOTAL A DISPERSAR AL PROVEEDOR:</strong> ${lote_info['total_capital'] + lote_info['total_comision_dns']:,.2f}</li>"
        cuerpo += "</ul>"
        cuerpo += "<p><em>Se adjunta layout CSV listo para dispersión en Fondeadora.</em></p>"
        cuerpo += "<p><em>Todas las transferencias van a cuentas del proveedor (ver detalles en CSV).</em></p>"
        cuerpo += "<p><em>También se adjuntan los comprobantes de pago originales del cliente.</em></p>"
        cuerpo += "</div>"
        
        # Sección de instrucciones para Tesorería
        cuerpo += "<hr>"
        cuerpo += "<div style='margin: 20px 0; padding: 15px; background: #fff3cd; border: 2px solid #ffc107;'>"
        cuerpo += "<h3>📋 Pasos para Tesorería</h3>"
        cuerpo += "<ol>"
        cuerpo += "<li><strong>Validar ingreso en firme</strong><ul>"
        cuerpo += "<li>Entra a tu banca donde se reciben los depósitos NetCash.</li>"
        cuerpo += "<li>Verifica que todos los depósitos de este lote estén ya en firme (no retenidos).</li>"
        cuerpo += "<li>Si algún depósito aún no está en firme, NO disperses ese caso hasta que se confirme.</li>"
        cuerpo += "</ul></li>"
        cuerpo += "<li><strong>Subir el layout a la banca para dispersión</strong><ul>"
        cuerpo += "<li>Utiliza el archivo CSV adjunto (layout Fondeadora) para cargar las transferencias.</li>"
        cuerpo += "<li>Verifica que los montos de capital y comisión DNS coincidan con el resumen del correo.</li>"
        cuerpo += "</ul></li>"
        cuerpo += "<li><strong>Responder a este correo con los comprobantes de dispersión</strong><ul>"
        cuerpo += "<li>Una vez realizadas las transferencias, responde a este mismo correo:</li>"
        cuerpo += "<li>Adjunta los comprobantes de pago de las transferencias (PDF/ZIP).</li>"
        cuerpo += "<li>Confirma si todas las solicitudes del lote quedaron dispersadas o si alguna quedó pendiente.</li>"
        cuerpo += "</ul></li>"
        cuerpo += "</ol>"
        cuerpo += "</div>"
